/**
 * Timeline Page
 * Migrated from TimelineView.js. All DOM created by JS.
 */

import { loadData, sendMessage } from '../shared/storage.js';
...
import SearchHelper from '../shared/SearchHelper.js';
class TimelinePage {
  constructor() {
  ...
  _openSelected() {
    if (this.selectedIndex >= 0 && this.currentResults[this.selectedIndex]) {
      ...
      sendMessage('openTab', { url: item.url });
      this._hideSearchDropdown();
      this.searchInput.value = '';
    }
  }
      ...
      item.addEventListener('click', async (e) => {
        ...
        switch (action) {
          ...
          case 'unmark':
            ...
            break;
          case 'open':
            sendMessage('openTab', { url: tabUrl });
            break;
          case 'copy':
  ...
  async _toggleMark(snapshotId, tabUrl, marked) {
    ...
    const result = await sendMessage('toggleTabMark', {
      snapshotId,
      tabUrl,
      marked
    });
  ...
  _expandSnapshot(btn) {
    ...
    remainingTabs.forEach(tab => {
      ...
      tabRow.addEventListener('click', (e) => {
        if (e.button === 2) return;
        sendMessage('openTab', { url: tab.url });
      });
  ...
  async _restoreSnapshot(snapshotId) {
    const result = await sendMessage('restoreSnapshot', { snapshotId });
    if (result.success) {
      await this.loadData();
      this.render();
    }
  ...
  async _deleteSnapshot(snapshotId) {
    ...
    await sendMessage('deleteTimelineSnapshot', { snapshotId });
    await this.loadData();
    this.render();
  }
  ...
  async _restoreAllSnapshots() {
    ...
    for (const snapshot of snapshotsToRestore) {
      for (const tab of snapshot.tabs) {
        await sendMessage('openTab', { url: tab.url });
      }
    }
  }
  ...
  async _clearAllSnapshots() {
    ...
    for (const snapshot of this.snapshots) {
      await sendMessage('deleteTimelineSnapshot', { snapshotId: snapshot.id });
    }
    await this.loadData();
    this.render();
  }
  ...
  _importData() {
    importData(async (data) => {
      ...
      const importResult = await sendMessage('importTimelineSnapshots', {
  ...
  async _extractMarkedAsGroup() {
    ...
    const result = await sendMessage('extractMarkedAsGroup', {
      markedTabs
    });
