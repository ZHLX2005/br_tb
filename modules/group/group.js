/**
 * Group Page
 * Migrated from GroupView.js. All DOM created by JS.
 */
...
class GroupPage {
  ...
  async _handleDeleteButtonClick(e) {
    ...
    if (tabId && groupId) {
      await sendMessage('deleteTab', { tabId, groupId });
    }
  }
  ...
  async _handleDropBoard(el, target, source, sibling) {
    ...
    // 更新看板顺序到存储
    await sendMessage('updateBoardOrder', { boardOrder });
  }
  ...
  async _handleOpenAll(e) {
    ...
    await sendMessage('openGroup', { groupId });
  }
  ...
  async _handleDeleteGroup(e) {
    ...
    await sendMessage('deleteGroup', { groupId });
    await this.loadData();
    this.render();
  }
  ...
  _setupGroupActionButtons() {
    ...
    if (openAllBtn) {
      openAllBtn.addEventListener('click', async () => {
        ...
        for (const group of this.groups) {
          await sendMessage('openGroup', { groupId: group.id });
        }
      });
    }
    ...
    if (clearAllBtn) {
      clearAllBtn.addEventListener('click', async () => {
        ...
        if (!confirmed) return;
        await sendMessage('clearAllGroups');
      });
    }
  ...
  _importData() {
    importData(async (data) => {
      ...
      await sendMessage('importGroupsAndTabs', {
        groups: data.groups,
        tabs: data.tabs
      });
  ...
  _startEditingGroupName(editBtn) {
    ...
    const saveEdit = async () => {
      ...
      const result = await sendMessage('updateGroupName', {
        groupId,
        newName
      });
  ...
  async _refreshAndSort() {
    ...
    await sendMessage('sortTabsByVisitCount');
