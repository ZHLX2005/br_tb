...
根据 Step 1 的回答，执行以下一种或多种操作：

| 经验类型 | 操作 | 示例 |
|---------|------|------|
| **新的架构事实** | 更新职责边界、依赖矩阵 | "原来 `auth.py` 已经拆成 `auth/login.py` + `auth/session.py` 了" |
| **新踩的坑** | 追加到错误案例表 | "WebSocket 断连不会自动重连，需要在 `onClose` 里加重试逻辑" |
| **发现新模式** | 新增正反案例 | "动态注册路由要在 `app.register()` 里声明,不要手动添加到 `routes/`" |
| **流程改进** | 优化 skill 的步骤 | "部署前要先跑 `migration`，skill 的部署步骤漏了这一步" |
| **上下文补全** | 新增例子或引用 | "测试文件在 `tests/` 目录下命名规则是 `test_*.py`" |
...
```
完成任务后
  ├─ 发现新踩坑？ → 立即记入 skill 错误案例
  ├─ 发现新模式？ → 立即记入 skill 正反案例
